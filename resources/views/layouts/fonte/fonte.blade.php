                                    ...
                                    <tbody>
                                        @foreach ($fontes as $fontes)
                                            <tr id="table_conteudo">
                                                <td data-label="Nome"><a
                                                        href="{{ $fontes->endereco }}">{{ $fontes->endereco }}</a></td>
                                                <td data-label="Email ">{{ $fontes->descricao }}</td>
                                                <td data-label="Acções ">
                                                    <button title="editar" class="button"><a class="button"
                                                        href="{{ route('fontes.edit',$fontes->id) }}"><i class="fas fa-edit edite_icon "
                                                            style="color: rgb(0, 0, 0)"></i></a></button>
                                                    <form method="POST" style="display: inline-block" action="{{ route('fontes.destroy', $fontes->id) }}" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('DELETE')
